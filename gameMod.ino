...
void makeQuestion() {
  ...
  if (currentQuestionI < s.n_questions) {                                   //se devo fare ancora domande
    ...
    currentAnswers = 0;                                                      //azzero il contatore delle risposte date fino ad ora
...
void waitAnswer() {
  ...
  if (currentCorrectAnswers < s.n_answers[currentQuestionI]) {    //se ci sono ancora risposte da dare
    ...
    if (pressedButton != -1) {                                        //se viene premuto un bottone
      int a = pressedButton;
      currentAnswers++;                                                 //aumento contatore risposte
      Serial3.println("Current Answers Added to: " + String(currentAnswers));
    ...
    if (currentAnswers == s.n_answers[currentQuestionI]) s.givenAnswersReport[currentQuestionI] = true;
...
void endGame() {
  ...
  for (int i = 0; i < s.n_questions - 1; i++) {
    ...
    //Serial3.print(String(s.questionResponseTime[i])); Serial3.print(" , ");
    Serial3.print(String(s.questionResponseTime[i])+" , ");
  }
  Serial3.write(s.questionResponseTime[s.n_questions - 1]); Serial3.print("*");
  Serial3.print("*H");
  for (int i = 0; i < s.n_questions - 1; i++) {
