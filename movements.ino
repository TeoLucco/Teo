...
void makeAnEight(){
  if(actual_movement==make_eight) {
    ...
    if(t > 2*PERIOD_MS/3 && t < 2*PERIOD_MS){
      triskar.run(FORWARD_SP,-ANGULAR_SP);
    }else
    if(t > 2*PERIOD_MS && t<8*PERIOD_MS/3)
      triskar.run(FORWARD_SP,ANGULAR_SP);
    else{
      stopMovement();
    }  
...
void makeAnEight2(){  
  float t=millis()-movStartTime; //msec
    ...
    if(t > 2*PERIOD_MS/3 && t < 4*PERIOD_MS/3){
...
void makeRunScaredBehind(){ //if dir is true the robot goes forward, if its false it goes backward
...
#define SAD_ANGULAR_SPEED 1.0f
void makeSad1(){ 
  float angular_speed=SAD_ANGULAR_SPEED;
...
#define SCARED_ANG_SP 1.5f //rad/s
void makeScaredRound(){
  
    if(movementI==0){
    if(startPosTh <0) {
      if(triskar.getPosTh()<startPosTh+PI)
        triskar.run(0.0f,-SCARED_ANG_SP);
      else movementI=1;
     }else{
       if(triskar.getPosTh()>startPosTh-PI)
          triskar.run(0.0f,SCARED_ANG_SP);
